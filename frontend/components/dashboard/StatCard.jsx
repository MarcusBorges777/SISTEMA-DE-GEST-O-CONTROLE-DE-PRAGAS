...
export default function StatCard({ title, value, subtitle, icon: Icon, color = 'blue', onClick, loading }) {
  ...
  return (
    <div
      ...
      aria-label={onClick ? `${title}: ${loading ? 'carregando' : value}. ${subtitle}` : undefined}
      className={`bg-white dark:bg-slate-800 rounded-2xl
        ...
        ${onClick
          ? `cursor-pointer hover:scale-[1.02] active:scale-[0.99]
             focus-visible:ring-2 ${c.ring} focus-visible:ring-offset-2`
          : ''
        }`}
    >
      <div className="flex items-center justify-between">
        <div className="min-w-0 flex-1">
          ...
          {loading ? (
            <div className="h-9 w-20 mt-2 bg-slate-200 dark:bg-slate-700 rounded-lg animate-pulse" aria-label="Carregando..." />
          ) : (
            <p
              className="text-3xl font-bold text-slate-900 dark:text-white mt-2 tabular-nums"
              aria-live="polite"
            >
              {value}
            </p>
        ...
        <div
          className={`w-14 h-14 rounded-2xl ${c.iconBg} flex items-center justify-center flex-shrink-0 ml-4`}
          aria-hidden="true"
        >
