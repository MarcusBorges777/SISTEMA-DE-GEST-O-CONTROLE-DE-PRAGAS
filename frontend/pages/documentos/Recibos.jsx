...
export default function Recibos() {
  ...
  const [reciboNumero, setReciboNumero] = useState(() => {
    try { return localStorage.getItem('receiptNumber') || '00001'; } catch { return '00001'; }
  });
  ...
  useEffect(() => {
    const onAfterPrint = () => {
      setReciboNumero(prev => {
        ...
        localStorage.setItem('receiptNumber', next);
        return next;
      });
    };
  ...
  const { salvandoPdf, handleSalvarPdf, handleSalvarCliente: salvarClienteHook } = useDocumentoActions({
    tipo: 'recibo',
    getNumeroDoc:   () => reciboNumero || '00001',
  ...
  const incrementNumero   = () => setReciboNumero(p => String((parseInt(p, 10) || 0) + 1).padStart(5, '0'));
  const decrementNumero   = () => setReciboNumero(p => { const n = parseInt(p, 10) || 0; return n > 1 ? String(n - 1).padStart(5, '0') : p; });
  const incrementGarantia = () => setGarantiaMeses(p => String((parseInt(p, 10) || 0) + 1));
  const decrementGarantia = () => setGarantiaMeses(p => { const n = parseInt(p, 10) || 0; return n > 0 ? String(n - 1) : '0'; });
    ...
    <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-4xl border border-blue-200 mb-8 no-print">
      <div className="flex justify-between items-center mb-6 border-b pb-2">
        <h3 className="font-bold text-lg text-blue-900 flex items-center gap-2">
          <Edit3 size={20} /> Editar Informações do Recibo
        </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1.5">Nº Recibo</label>
                  <div className="flex gap-2">
                    <input type="text" value={reciboNumero} onChange={e => setReciboNumero(e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-[#3b4b73] mb-1.5">Garantia (Meses)</label>
                  <div className="flex gap-2">
                    <input type="text" value={garantiaMeses} onChange={e => setGarantiaMeses(e.target.value)}
                      className="w-full p-2.5 text-center bg-white border border-gray-200 rounded-md text-sm font-semibold text-blue-700 focus:ring-2 focus:ring-blue-100 outline-none shadow-sm" />
                    <button onClick={decrementGarantia} className="flex items-center justify-center bg-red-50 hover:bg-red-100 border border-red-200 rounded-md w-11 shrink-0 text-red-500"><Minus size={16} /></button>
                    <button onClick={incrementGarantia}  className="flex items-center justify-center bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-md w-11 shrink-0 text-blue-500"><Plus size={16} /></button>
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#3b4b73] mb-1.5">Próxima Manutenção</label>
                  <input type="text" value={proximaManutencao} onChange={e => setProximaManutencao(e.target.value)}
      ...
      <div className="a4-page relative bg-white shadow-2xl p-[15mm] flex flex-col print:shadow-none print:m-0 overflow-hidden text-slate-800">
        ...
        <div className="flex justify-between items-end mb-3">
          <div className="min-w-0 flex-1 mr-4">
            <h2 className="text-3xl font-black text-[#254191] uppercase leading-none tracking-wide">RECIBO</h2>
        ...
        <div className="mt-auto">
          ...
          <div className="grid grid-cols-2 gap-6 mb-3 text-[10px]">
            ...
            <div>
              ...
              <p className="text-gray-600 italic font-medium leading-snug">
                Garantia técnica: {garantiaMeses} {parseInt(garantiaMeses) === 1 ? 'mês' : 'meses'}
                {proximaManutencao && ` • Próxima manutenção: ${proximaManutencao}`}
              </p>
            </div>
          </div>
