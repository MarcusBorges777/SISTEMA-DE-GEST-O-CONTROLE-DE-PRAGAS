/**
 * Gerador de Orçamentos - React Component
...
const QuoteApp = () => {
  ...
  const mascotConfig = { height: 360, x: -15, y: 225, opacity: 1 };
  ...
  // Formatar data por extenso para o backend
  const formatDateExtended = (dateStr) => {
    ...
    if (parts.length === 3) {
      ...
      const y = parts[2];
      return `${String(d).padStart(2, '0')} de ${meses[m] || ''} de ${y}`;
    }
    return dateStr;
  };
  ...
  const saveToBackend = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        DATA: formatDateExtended(date),
        servicos: items.filter(i => i.service).map(i => i.service).join(', ') || 'Servi\u00e7os de dedetiza\u00e7\u00e3o',
        valor_total: total.toFixed(2).replace('.', ','),
        valor_formatado: valorFormatado,
        escrito: '',
        validade: terms,
        servico_categoria: items[0]?.service || '',
        dependencias_servico: items.map(i => i.description).filter(Boolean).join('; '),
      };
      const res = await fetch('/gerar-orcamento', {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        a.href = url;
        a.download = `ORCAMENTO_${clientData.nome || 'Cliente'}.docx`;
        a.click();
        URL.revokeObjectURL(url);
        setSaveMessage('Or\u00e7amento gerado e salvo com sucesso!');
      } else {
    ...
    <div className="min-h-screen bg-slate-100 p-4 md:p-8 flex flex-col items-center font-sans text-slate-900">
      ...
      <div className="mb-4 flex flex-wrap justify-between items-center gap-4 print:hidden w-full max-w-[210mm] bg-white p-5 rounded-2xl shadow-lg border border-slate-200">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button onClick={() => fileInputMascot.current.click()} className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2.5 rounded-xl hover:bg-indigo-700 transition-all font-bold text-sm">
      ...
      <div id="a4-document" className="bg-white w-[210mm] min-h-[297mm] shadow-[0_0_60px_rgba(0,0,0,0.1)] relative overflow-hidden flex flex-col p-[15mm] text-slate-800">
        ...
        <section className="bg-blue-50/30 p-3 rounded-lg border border-blue-100 w-full shadow-sm mb-10 relative z-10">
          <h3 className="flex items-center gap-2 text-[#254191] font-bold uppercase text-[9px] mb-2 border-b border-blue-200 pb-1 italic">
            <Icons.User size={12} /> Cliente / Contratante
          </h3>
          <div className="text-[10px] space-y-1 text-gray-700">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">
              ...
              <div className="md:border-l md:border-blue-200 md:pl-4 space-y-1">
                <div>
                    <label className="font-bold uppercase text-[8px] tracking-tighter text-blue-800 flex items-center gap-1"><Icons.MapPin size={8}/> Endereço:</label>
                ...
                <div className="pt-1">
                    <label className="font-bold uppercase text-[8px] tracking-tighter text-blue-800 flex items-center gap-1"><Icons.Briefcase size={8}/> Atividade Econômica:</label>
        ...
        <section className="mb-10 relative z-10">
            <table className="w-full text-left border-collapse border border-blue-200 text-[10px] shadow-sm rounded-lg overflow-hidden">
                <thead className="bg-[#254191] text-white uppercase font-black text-[9px]">
                    <tr>
        ...
        <div className="mt-auto relative z-10">
            <section className="bg-[#254191] text-white py-3 px-5 rounded-lg flex justify-between items-center shadow-md border-b-4 border-blue-900/50 mb-10">
            ...
            <div className="text-center mb-10">
                ...
                <div className="mt-2 bg-blue-50/50 border border-blue-100 rounded-xl p-3 inline-block">
        ...
        {mascot && (
          ...
          >
            <img src={mascot} alt="Mascote" style={{ height: `${mascotConfig.height}px` }} className="w-auto object-contain drop-shadow-xl" />
          </div>
        )}
      </div>
    </div>
...
if (rootElement) {
  ReactDOM.createRoot(rootElement).render(<QuoteApp />);
}
